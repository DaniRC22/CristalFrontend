...
export default function Cart() {
  ...
  return (
    <>
      <Helmet><title>Carrito — KAP Equipamiento Comercial</title></Helmet>
      <div className="max-w-4xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Carrito de compras</h1>

        <div className="space-y-3 mb-6">
          {items.map((item) => (
            <div key={item.cart_key} className="flex items-center gap-4 bg-white border border-gray-100 rounded-xl p-4">
              {item.image_url && (
                <img src={item.image_url} alt={item.name} className="w-16 h-16 object-cover rounded-lg shrink-0" />
              )}
              <div className="flex-1 min-w-0">
                <Link to={`/productos/${item.slug}`} className="text-sm font-medium text-gray-900 hover:underline line-clamp-1">{item.name}</Link>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => updateQuantity(item.cart_key, Math.min(item.quantity + 1, item.stock))} disabled={item.quantity >= item.stock} className="p-1 rounded border hover:bg-gray-50 disabled:opacity-40"><Plus size={14} /></button>
              </div>
              <p className="w-24 text-right font-semibold text-sm">{formatPrice(item.price * item.quantity)}</p>
              <button onClick={() => removeItem(item.cart_key)} className="text-gray-400 hover:text-red-500"><Trash2 size={18} /></button>
            </div>
          ))}
        </div>
        ...
        <div className="bg-gray-50 rounded-xl p-5 flex items-center justify-between">
          ...
          <button onClick={() => navigate('/checkout')} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors">
            Continuar compra
          </button>
        </div>
      </div>
